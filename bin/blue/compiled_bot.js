...
(function (bc19) {
    var Pilgrim = (function () {
        ...
        Pilgrim.hasAdjacentUnit = function (unit) {
            var x;
            var y;
            var u;
            for (var i = 0; i < 8; i++) {
        ...
        Pilgrim.turn = function () {
            ...
            switch ((Pilgrim.state_$LI$())) {
                ...
                case bc19.Pilgrim.State.DEPOSIT:
                    ...
                    next_move = null;
                    if (Pilgrim.bot.path.getSquareDist$int$int$int$int(Pilgrim.bot.me.x, Pilgrim.bot.me.y, Pilgrim.target.x, Pilgrim.target.y) <= Pilgrim.bot.v_radius_sq) {
                        next_move = Pilgrim.bot.getMove(bot_pos, Pilgrim.target, Pilgrim.bot.fuel);
                        return Pilgrim.bot.move(next_move.x, next_move.y);
                    }
                    if (next_move == null) {
                        if ((adj = Pilgrim.hasAdjacentUnit(Pilgrim.bot.SPECS.PILGRIM)) != null) {
                            return Pilgrim.bot.give(adj.x, adj.y, Pilgrim.bot.me.karbonite, Pilgrim.bot.me.fuel);
                        }
                        return null;
                    }
...
(function (bc19) {
    ...
    Params.INITIAL_PILGRIMS_FRAC = 0.5;
    bc19.Params = Params;
...
(function (bc19) {
    var Castle = (function () {
        ...
        Castle.turn = function () {
            ...
            if (Castle.numPilgrims < (Castle.bot.num_mines * bc19.Params.INITIAL_PILGRIMS_FRAC))
...
(function (bc19) {
    ...
    var Path = (function () {
        ...
        Path.prototype.getSquareDist$bc19_Point2D$bc19_Point2D = function (t, s) {
            return (t.x - s.x) * (t.x - s.x) + (t.y - s.y) * (t.y - s.y);
        };
        Path.prototype.getSquareDist$int$int$int$int = function (tx, ty, sx, sy) {
            return (tx - sx) * (tx - sx) + (ty - sy) * (ty - sy);
        };
        Path.prototype.getSquareDist = function (tx, ty, sx, sy) {
            if (((typeof tx === 'number') || tx === null) && ((typeof ty === 'number') || ty === null) && ((typeof sx === 'number') || sx === null) && ((typeof sy === 'number') || sy === null)) {
                return this.getSquareDist$int$int$int$int(tx, ty, sx, sy);
            }
            else if (((tx != null && tx instanceof bc19.Point2D) || tx === null) && ((ty != null && ty instanceof bc19.Point2D) || ty === null) && sx === undefined && sy === undefined) {
                return this.getSquareDist$bc19_Point2D$bc19_Point2D(tx, ty);
            }
            else
                throw new Error('invalid overload');
        };
        ...
        Path.prototype.costH = function (x, y) {
            if (this.bot.robotMemMap[y][x] === -1)
                return 0;
            if (this.bot.robotMemMap[y][x] === 0)
                return 0;
            var age = this.bot.me.turn - this.bot.turnSeen[y][x];
            if (age < 3)
                return 2;
            else if (age < 5)
                return 1;
                    ...
                    for (var dx = -3; dx <= 3; dx++) {
                        for (var dy = -3; dy <= 3; dy++) {
                            ...
                            if ((t.x === x && t.y === y) || (this.isPointInBounds$int$int(x, y) && (cost < minimap[y][x] || minimap[y][x] === 0) && this.map[y][x] && this.r_map[y][x] <= 0)) {
                                minimap[y][x] = cost + this.costH(y, x);
    ...
    var MyRobot = (function (_super) {
        ...
        MyRobot.prototype.turn = function () {
            ...
            if (this.run_once) {
                ...
                this.getFuelList();
                this.num_mines = this.fuelList.length + this.karboniteList.length;
            }
        ...
        MyRobot.prototype.getClosestFuel = function () {
            ...
            for (var i = 0; i < this.fuelList.length; i++) {
                ...
                if (dist_sq < min_dist && this.map[ty][tx] && this.robotMemMap[ty][tx] <= 0) {
                    this.log("memmap tx ty " + this.robotMemMap[ty][tx] + " " + tx + ", " + ty);
                    min_dist = dist_sq;
                    index = i;
                }
            }
